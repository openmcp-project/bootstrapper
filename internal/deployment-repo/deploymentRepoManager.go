...
func (m *DeploymentRepoManager) CommitAndPushChanges(_ context.Context, commitMessage string) error {
	logger := log.GetLogger()

	logger.Info("Committing and pushing changes to deployment repository")

	err := CommitChanges(m.gitRepo, commitMessage, "openmcp", "noreply@openmcp.cloud")
	if err != nil {
